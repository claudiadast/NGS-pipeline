"""
Module to assist building syntactically correct StepFunctions State Machines using Python's type system.

Comparison Operator classes are generated dynamically from resp. Base classes and added to the module's __dict__.
...
class StringPredicate(Predicate):
    def __init__(self, op, var, val):
        assert isinstance(val, str), 'StringPredicates requires value of type str'
        super().__init__(op, var, val)


class NumericPredicate(Predicate):
    def __init__(self, op, var, val):
        assert isinstance(val, numbers.Number), 'NumericPredicates requires value of type Number'
        super().__init__(op, var, val)
