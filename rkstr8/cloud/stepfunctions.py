...
class PipelineSpecification(ABC):
    ...
    def build_substitutions(self):
        ...
        These substitutions are for keys used in the machine definition, and get replaced by Cloudformation.
        :return:
        """
        pass
    ...
    def build_template_params(self):
        """
        Holds the CFN template parameters for the stack supporting this pipeline, or otherwise gets those parameters,
    ...
    def build_input(self, stack):
        """
        Must build an input object, and set self.mx_input, which is passed as input to the machine
    ...
    #
    # TODO: delete or pull into new/existing Module or Class with SFN execution responsibility
    #
    # E.g. Use the StepFunctionsMachine class below?
    # Note, this _uses_ the StepfunctionsMachine.start method!
    #

    def execute(self, stack, mx_logical_resource_id):
        """
        Gets the ARN of the machine from the provided stack, based on its LogicalResourceId.
        Then executes the machine, given the input defined by self.build_input

        :param stack: boto3.CloudFormation.Stack
        :param mx_logical_resource_id: string:
        :return:
        """
        machine_arn = stack.Resource(mx_logical_resource_id).physical_resource_id
...
class StepFunctionsMachine:
    ...
    @staticmethod
    def start(machine_arn, input, exec_name=None, sfn_client=BotoClientFactory.client_for(Service.STEPFUNCTIONS)):
    ...
    @staticmethod
    def execution_status(execution_arn, sfn_client=BotoClientFactory.client_for(Service.STEPFUNCTIONS)):
    ...
    @staticmethod
    def get_output(execution_arn, sfn_client=BotoClientFactory.client_for(Service.STEPFUNCTIONS)):
        response = sfn_client.describe_execution(executionArn=execution_arn)
...
class AsyncPoller:
    ...
    def __init__(self, async_task, pollr_task, faild_task, succd_task, stats_path, pollr_wait_time):
        """
        ...
        The methods of this class wire the provided components with the intrinsic components of the polling system.
        ...
        :param pollr_task: The task which polls for asynch task completion. Must be a stepfunctions_mx.Task instance.
